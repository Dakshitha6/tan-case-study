...
export const TextRevealCard = ({
  ...
  function touchMoveHandler(event: React.TouchEvent<HTMLDivElement>) {
    event.preventDefault();
    const clientX = event.touches[0]?.clientX; // Added optional chaining for safety
    if (cardRef.current) {
      const relativeX = clientX - left;
      setWidthPercentage((relativeX / localWidth) * 100);
    }
  ...
  return (
    <div
      ...
      ref={cardRef}
      className={cn("w-[70rem] p-8 relative overflow-hidden", className)}
    >
      {children}

      <div className="h-40 relative flex items-center overflow-hidden text-center">
        <motion.div
          style={{
            width: "100%",
          }}
